...
GeoBeans.Layer.BarChartLayer.prototype.getMinMaxValue = function(){
	var max = null;
	var min = null;
	var chartField = null;
	var feature = null;
	...
	for(var i = 0; i < this.baseLayerFields.length; ++i){
		...
		// 	continue;
		// }
		var fieldMax = -Number.MAX_VALUE;
		var fieldMin = Number.MAX_VALUE;
...
GeoBeans.Layer.BarChartLayer.prototype.drawLayer = function(){
	...
	for(var i = 0; i < this.features.length; ++i){
		...
		for(var j = 0; j < length; ++j){
			...
			var v = chartFeature.getValue(chartField);
			if(isValid(v)){
