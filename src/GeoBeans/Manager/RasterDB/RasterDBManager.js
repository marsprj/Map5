GeoBeans.RasterDBManager = GeoBeans.Class({
	...
	// 获取缩略图
	getThumbnails : function(sourceName,path,callback){
		if(path == null || sourceName == null){
			if(callback != null){
				callback("params is invalid");
			}
		...
		var that = this;
		var params = "service=" + this.service + "&version="
					+ this.version + "&request=GetThumbnail"
					+ "&sourceName="+ sourceName + "&path=" + path;
		that.currentPath = path;
		$.ajax({
			type 	: "get",
			...
			success	: function(xml, textStatus){
				var result = that.parseGetThumbnails(xml);
				if(callback != undefined){
					callback(result);
				}
			},			 
	...
	parseGetThumbnails : function(xml){
		if(xml == null){
			return null;
		}
		var list = [];
		var that = this;
		$(xml).find("Rasters>Raster").each(function(){
			var thumb = that.parseGetThumbnail(this);
			if(thumb != null){
				list.push(thumb);
			}
		});
	...
	getThumbnail : function(sourceName,path,rasterName,callback){
		if(path == null || sourceName == null){
			if(callback != null){
				callback("params is invalid");
			}
		...
		$.ajax({
			...
			success	: function(xml, textStatus){
				var result = that.parseGetThumbnail(xml);
				if(callback != undefined){
					callback(result);
				}
			},			 
	...
	parseGetThumbnail : function(xml){
		if(xml == null){
			return null;
		}
		var name = $(xml).find("Name").text();
		var thumb = $(xml).find("Thumbnail").attr("xlink");
		return {
			name : name,
			thumb : thumb
		};
