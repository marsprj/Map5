GeoBeans.GPSManager = GeoBeans.Class({
	...
	parseGetCapabilities : function(xml){
		...
		$(xml).find("OperationsSet").each(function(){
			...
			var operList = [];
			operationSetXML.find("ows\\:Operation").each(function(){
	...
	getSpatialReferenceBySrid : function(srid,callback){
		...
		$.ajax({
			...
			success	: function(xml, textStatus){
				var srid = that.parseSpatialReferences(xml);
				if(callback != null){
					callback(srid);
				}
			},			 
	...
	demAspect : function(inputSourceName,inputRasterName,inputPath,
		...
		$.ajax({
			...
			success	: function(xml, textStatus){
				var result = that.parseDemAspect(xml);
				if(callback != null){
					callback(result);
				}
			},			 
	...
	delaunay : function(sourceName,typeName,inputZField,outputSourceName,outputTypeName,callback){
		...
		if(inputZField != null){
			params += "&inputZField=" + inputZField;
		}
		$.ajax({
			type 	: "get",
			url 	: this.server,
	...
	// 导入CSV文件
	csvImport : function(sourceName,typeName,csvPath,csvName,srid,callback){
		if(sourceName == null || typeName == null || csvPath == null
			||csvName == null ){
			if(callback != null){
		...
		var that = this;
		var params = "service=" + this.service + "&version=" + this.version
				+ "&request=CsvImport&sourceName=" + sourceName
				+ "&typeName=" + typeName + "&csvPath=" + csvPath
				+ "&csvName=" + csvName;
		if(srid != null){
			params += "&srid=" + srid;
		}
		$.ajax({
			...
			success	: function(xml, textStatus){
				var result = that.praseCsvImport(xml);
				if(callback != null){
					callback(result);
				}
			},			 
	...
	praseCsvImport : function(xml){
		var result = $(xml).find("CsvImport").text();
		if(result.toLowerCase() == "success"){
			return "success";
		}
		var exception = $(xml).find("ExceptionText").text();
