GeoBeans.FilterWriter = GeoBeans.Class({
	...
	writeSpatialFilter : function(xml,filter){
		...
		var spatialXML = null;
		if(operator == GeoBeans.SpatialFilter.OperatorType.SpOprBBox){
			spatialXML = this.writeBBoxFilter(xml,filter);
		}else if(operator == GeoBeans.SpatialFilter.OperatorType.SpOprIntersects){
	...
	writeBBoxFilter : function(xml,bboxFilter){
		...
		var propXML = this.writeExpression(xml,propName);
