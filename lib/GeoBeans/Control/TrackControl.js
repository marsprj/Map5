GeoBeans.Control.TrackControl = GeoBeans.Class(GeoBeans.Control, {
		...
		var onmouseclick = function(evt){
			...
			var onmousedbclick = function(evt){
				...
				if( (callback!=null) && (callback!='undefined')){
					...
					// 	linePoints = points.slice(0,points.length - 2);
					// 	callback(that.buildLineString(linePoints));
					// }
	...
	trackBufferLine : function(callback,buffer){
		var that = this;
		var points = [];
		var db_points = [];
		this.map.saveSnap();
		...
		var onmouseclick = function(evt){
			...
			var onmousemove = function(evt){
				// that.map.restoreSnap();
				// var bufferRenderer = buffer.layer.bufferRenderer;
				// bufferRenderer.drawLineString()
				that.drawLine(points, evt.layerX,evt.layerY);
				that.drawPoints(points, evt.layerX,evt.layerY);
			};
			...
			var onmousedbclick = function(evt){
				...
				if( (callback!=null) && (callback!='undefined')){
					...
					// 	callback(that.buildLineString(linePoints));
					// }
					callback(that.buildLineString(points),buffer);
				}
				// points = [];
				db_points = [];
				points = [];
