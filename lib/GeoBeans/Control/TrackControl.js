GeoBeans.Control.TrackControl = GeoBeans.Class(GeoBeans.Control, {
	...
	trackRect : function(callback,map,layer,callback_u){
		var that = this;
		...
		var onmousedown = function(evt){
			...
			var onmouseup = function(evt){
				...
				if(callback != null && callback != 'undefined'){
					callback(rect,map,layer,callback_u);
				}
				point_b = null;
				point_e = null;
			};
	...
	trackInfo : function(callback,layer,map,callback_u){
		...
		var onmousedown = function(evt){
			...
			that.map.canvas.addEventListener("mouseup", onmouseup);
			// var point = new GeoBeans.Geometry.Point(evt.layerX,evt.layerY);
			// if(callback != null && callback != undefined){
			// 	callback(point,layer,map);
			// }
			that.onMouseUp = onmouseup;
		};
		this.map.canvas.addEventListener("mousedown", onmousedown);
