...
GeoBeans.Map = GeoBeans.Class({
	...
	initialize: function (server,id,name,extent,srid,viewer) {
		...
		// 授权时间
		this.authTime = new Date("2118-05-26 00:00:00");
	},
	...
	drawLayersSnap_2 : function (zoom,p1,p2) {
		var lt = this.transformation.toScreenPoint(p1.x,p1.y);
		var rb = this.transformation.toScreenPoint(p2.x,p2.y);
		...
		if(this.snap != null){
			var newCanvas = $("<canvas>")
			    ...
			    .attr("height", this.snap.height)[0];
			newCanvas.getContext("2d").putImageData(this.snap, 0, 0);
			this.renderer.context.drawImage(newCanvas,lt.x,lt.y,width,height);
		}
		...
		if(this.baseLayerSnap != null){
			var baseLayerCanvasNew = $("<canvas>")
			    ...
			    .attr("height", this.baseLayerSnap.height)[0];
			baseLayerCanvasNew.getContext("2d").putImageData(this.baseLayerSnap, 0, 0);
			this.baseLayer.renderer.context.drawImage(baseLayerCanvasNew,lt.x,lt.y,width,height);
		}
	},
