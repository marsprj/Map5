...
//	},
	...
	dragMap : function(){
		...
		var onmousedown = function(evt){
			var resolution = map.viewer.getWidth() / map.canvas.width;
			...
			var onmousemove = function(evt){
				if(draging){
					...
					o_x = (d_x - m_x) * resolution;
					o_y = (m_y - d_y) * resolution;
					
					map.offset(o_x, o_y);
					console.log(map.viewer.xmin);
					map.drawCache();
	...
	addEventListener : function(event, handler){
		...
		this.canvas.addEventListener(event, function(evt){
			...
			var args = new GeoBeans.Events.MouseArgs();
			args.buttn = null;
			args.X = x;
			args.Y = y;
			args.mapX = mp.x;
