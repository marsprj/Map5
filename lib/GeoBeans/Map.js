...
GeoBeans.Map = GeoBeans.Class({
	...
	initialize: function (server,id,name,extent,srid,viewer) {
		...
		var mapCanvasHtml = "<canvas id='" + canvasID + "' height='" 
							+ $("#" + id).height() + "' width='" 
							+ $("#" + id).width() + "'></canvas>";
	...
	tooltip : function(point,content){
		...
		// var styleHtml = 'position: absolute;  border: 0px solid rgb(51, 51, 51);' 
		// + 'white-space: nowrap; z-index: 9999999; transition: left 0.4s cubic-bezier(0.23, 1, 0.32, 1),'
		// + ' top 0.4s cubic-bezier(0.23, 1, 0.32, 1); border-radius: 4px; color: rgb(255, 255, 255);'
		// + ' font-style: normal; font-variant: normal; font-weight: normal; font-stretch: normal; font-size: 14px; '
		// + ' font-family: \'Microsoft YaHei\'; line-height: 21px; padding: 5px; background-color: rgba(50, 50, 50, 0.701961);';
		var position = 'left:' + spt.x + "px;top:" + spt.y + "px";
		// if(this.mapDiv.find(".map5-tooltip").length == 0){

		// }
		// var tooltipHtml = '<div  class="map5-tooltip" style="' + styleHtml + position + '">' + content + "</div>";
		// this.closeTooltip();
		// this.mapDiv.append(tooltipHtml);
	...
	closeTooltip : function(){
		...
		// this.mapDiv.find(".map5-tooltip").remove();
	},
	...
	registerRippleHitEvent : function(name,content){
		...
		this.canvas.addEventListener('mousemove', this.hitRippleEvent);		
	},
	...
	unRegisterRippleHitEvent : function(name){
		...
		if(this.hitRippleLayers.length == 0){
			this.canvas.removeEventListener('mousemove',this.hitRippleEvent);
