...
GeoBeans.Map = GeoBeans.Class({
	...
	addLayer : function(layer){
		...
		if(layer instanceof GeoBeans.Layer.ChartLayer ){
			...
			if(!(layer instanceof GeoBeans.Layer.ClusterLayer)){
	...
	//拉框查询
	queryByRect : function(layerName,style,callback,maxFeatures,offset){
		if(layerName == null){
			return;
		}
	...
	trackRect_callback : function(rect,map,layer,style,callback_u){
		...
		var offset = map.queryLayer.offset;
		var features = layer.getFeatureBBoxGet(rect,maxFeatures,offset);
		map.queryLayer.setFeatures(features);
		callback_u(features);
		map.queryLayer.maxFeatures = null;
		map.queryLayer.offset = null;
	},
	...
	getFeatureFilter : function(layerName,filter,maxFeatures,offset,fields,style,callback){
		var layer = this.getLayer(layerName);
		if(layer == null){
			if(callback != null){
		...
		layer.getFeatureFilter(filter,maxFeatures,offset,fields,callback);
	},
