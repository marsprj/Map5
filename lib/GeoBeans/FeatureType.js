GeoBeans.FeatureType = GeoBeans.Class({
	...
	buildGetFeaturesXMLWithin : function(mapName,sourceName,point){
		var xml = "";
	...
	// 较为通用的查询fitlera
	getFeaturesFilter : function(mapName,sourceName,filter,maxFeatures,offset){
		var that = this;
		var url = this.workspace.server;

		var xml = this.buildGetFeatureFilterXML(mapName,sourceName,
			filter,maxFeatures,offset);
	...
	getFeatureFilterOutput : function(mapName,sourceName,filter,maxFeatures,offset){
		var url = this.workspace.server; 
		// var params = this.buildGetFeatureFilterXML(mapName,sourceName,
		// 	filter,maxFeatures,offset,output);
		// // return url + "?" + params;
		// return {
		// 	type : "post",
		// 	url  : url,
