GeoBeans.Layer.AQIChartLayer = GeoBeans.Class(GeoBeans.Layer.ChartLayer,{
	...
	initialize : function(name,dbName,tableName,chartField,labelField,flagField,timePoint,chartOption){
		this.name = name;
		this.dbName = dbName;
	...
	showTips : function(){
		if(this.features == null || this.chartOption == null
			|| this.colors == null || this.minMaxValue == null){
			return;
		}
		...
		// 渐变色
		var colorRangeMap = new GeoBeans.ColorRangeMap(this.colors[0],this.colors[1],
			this.minMaxValue.min,this.minMaxValue.max);
