GeoBeans.Layer.AQIChartLayer = GeoBeans.Class(GeoBeans.Layer.ChartLayer,{
	...
	setMap : function(map,isTimeline){
	...
	getFeatures : function(){
		...
		// 采用异步模式
		this.featureType.getFeaturesFilterAsync2(null,
			this.dbName,filter,null,null,[this.chartField,geomField,
			this.labelField,this.flagField],null,this,this.getFeatures_callback);
	...
	getFeatures : function(){
		...
		var geomField = this.featureType.geomFieldName;
		this.featureType.getFeaturesFilterAsync2(null,
			this.dbName,timeFilter,null,null,[this.chartField,geomField,
			this.labelField,this.flagField],null,this,this.getFeatures_callback);
	...
	drawAQIChart : function(){
		var type = this.chartOption.type;
		if(type == "point"){
			this.renderer.context.globalCompositeOperation = "lighter";
			this.drawByValue();
		}else if(type == "tip"){
			// this.renderer.context.globalCompositeOperation = "lighter";
			this.renderer.context.globalCompositeOperation = "source-over";
			this.showTips();
			this.drawLayer();			
		}
	},
	...
	registerHitEvent : function(infoWindowContent,callback){
		...
		this.map.mapDiv[0].addEventListener('click', this.hitEvent);
		this.map.mapDiv[0].addEventListener('mousemove', this.moveEvent);
	},

	unRegisterHitEvent : function(){
		this.map.mapDiv[0].removeEventListener('click',this.hitEvent);
		this.map.mapDiv[0].removeEventListener('mousemove',this.moveEvent);
		this.hitEvent = null;
		this.moveEvent = null;
	},
	...
	getTipInfo : function(point){
		...
		for(var i = this.tipInfos.length; i >=0; i--){
			...
			if(type == "point"){
				center = tipInfo.center;
				radius = tipInfo.radius;
	...
	drawByValue : function(){
		...
		var geometry = null,f = null,values = null,value = null;
		var radius = null,color = null,radiusColor = null;
		...
		for(var i = 0 ; i < features.length;++i){
			...
			color = this.colorFunctionByColorMap(value);


			radiusColor = new GeoBeans.Color();
			radiusColor.setByHex(color,aplha);
			this.renderer.context.fillStyle = radiusColor.getRgba();
			spt = this.map.transformation.toScreenPoint(geometry.x,geometry.y);

			this.renderer.context.beginPath();
			this.renderer.context.arc(spt.x,spt.y,radius,0,Math.PI*2);
	...
	colorFunctionByColorMap : function(value){
		var minMaxValue = this.minMaxValue;
		var min = minMaxValue.min;
		var max = minMaxValue.max;

		var colorMap = this.colorMap;
		var begin = this.colorMap.startColor;
		var end = this.colorMap.endColor;
		if(value < min){
			return begin;
		}
		if(value > max){
