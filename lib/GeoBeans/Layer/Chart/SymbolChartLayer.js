GeoBeans.Layer.SymbolChartLayer = GeoBeans.Class(GeoBeans.Layer.ChartLayer,{
	...
	// 获取样式,根据数值来划分
	getStyle : function(){
		var style = new GeoBeans.Style.FeatureStyle("symbol",GeoBeans.Style.FeatureStyle.GeomType.Polygon);
		var rules = [];
		var rule = new GeoBeans.Rule();
		var symbolizer = new GeoBeans.Symbolizer.PolygonSymbolizer();
		var color = this.chartOption.color;
		if(color != null){
			symbolizer.fill.color = this.chartOption.color;
		}
	...
	addLegendByLevel : function(){
		...
		var html = "<div class='chart-legend chart-symbol-legend' id='" + this.name + "' style='left:" + left + "px'>";
