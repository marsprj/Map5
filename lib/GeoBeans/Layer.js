GeoBeans.Layer = GeoBeans.Class({
	...
	// 			var newLayer = this.map.layers[newIndex];
	// 			if(newLayer != undefined){
	// 				for(var j = i; j < newIndex; ++j){
	// 					this.map.layers[j] = this.map.layers[j+1];
	// 				}
	// 				this.map.layers[newIndex] = this;
	// 			}
	// 			return;
	// 		}
	// 	}
	...
	up : function(callback){
		if(!(this instanceof GeoBeans.Layer.DBLayer)){
			if(callback != null){
				callback("不支持此操作");
			}
			return;
		}
		var layers = this.map.groupLayer.getLayers();
