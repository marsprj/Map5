GeoBeans.StyleWriter = GeoBeans.Class({
	...
	writeFeature : function(style){
		...
		$('sld\\:StyledLayerDescriptor',xml).append(userLayerNode);
		return xml;
	},
	...
	writeUserStyle: function(xml,style){
		...
		if(styleClass != null){
			var styleClassXML = xml.createElement("sld:StyleClass");
			$(styleClassXML).text(styleClass);
			$(userStyleXML).append(styleClassXML);
			if(styleClass == GeoBeans.Style.FeatureStyle.StyleClass.DOTDENSITY){
				var field = style.field;
				if(field != null){
					var styleFieldXML = xml.createElement("sld:StyleField");
					$(styleFieldXML).text(field);
					$(userStyleXML).append(styleFieldXML);
				}
			}
		} 
		return userStyleXML;
	...
	writeLineSymbolizer : function(xml,lineSymbolizer){
		...
		var symbol = lineSymbolizer.symbol;
		if(symbol != null){
			var symbolXML = this.writeSymbol(xml,symbol);
			$(lineSymbolizerXML).append(symbolXML);
		}
		...
		var fill = lineSymbolizer.fill;
		if(fill != null){
			var fillXML = this.writeFill(xml,fill);
			$(lineSymbolizerXML).append(fillXML);
		}
	...
	writePolygonSymbolizer: function(xml,polygonSymbolizer){
		...
		var symbol = polygonSymbolizer.symbol;
		if(symbol != null){
			var symbolXML = this.writeSymbol(xml,symbol);
			$(polygonSymbolizerXML).append(symbolXML);
		}
